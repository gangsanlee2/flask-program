...
from src.cmm.const.path import CRIME_DATA_CTX, CRIME_SAVE_CTX
...
class CrimeService:
    def __init__(self):
        self.crime = pd.read_csv(CRIME_DATA_CTX+'crime_in_seoul.csv')
        cols = ['절도 발생', '절도 검거', '폭력 발생', '폭력 검거']
        self.crime[cols] = self.crime[cols].replace(',', '', regex=True).astype(int)
        self.cctv = pd.read_csv(CRIME_DATA_CTX+"cctv_in_seoul.csv")
        self.ls = [self.crime,self.cctv]
        self.pop = pd.read_excel(CRIME_DATA_CTX+"pop_in_seoul.xls", header=1,
                                 usecols=['자치구','합계','한국인','등록외국인','65세이상고령자'], skiprows=[2])
        self.crime_rate_columns = ['살인검거율', '강도검거율', '강간검거율', '절도검거율', '폭력검거율']
        self.crime_columns = ['살인', '강도', '강간', '절도', '폭력']
        self.arrest_columns = ['살인 검거', '강도 검거', '강간 검거', '절도 검거', '폭력 검거']
        self.us_states = CRIME_DATA_CTX+'us-states.json'
        self.us_unemployment = pd.read_csv(CRIME_DATA_CTX+'us_unemployment.csv')
        self.kr_states = CRIME_DATA_CTX+'kr-state.json'
    ...
    def save_police_pos(self): #nominal
        ...
        crime.to_csv(CRIME_SAVE_CTX+'police_pos.csv', index=False)
        crime.to_pickle(CRIME_SAVE_CTX+'police_pos.pkl')
        df = pd.read_pickle(CRIME_SAVE_CTX+'police_pos.pkl')
        print(df)
    ...
    def save_cctv_pop(self): # ratio -> nominal
        ...
        cctv_pop.to_pickle(CRIME_SAVE_CTX+"cctv_pop.pkl")
        """
    ...
    def save_police_norm(self):
        police_pos = pd.read_pickle(CRIME_SAVE_CTX+'police_pos.pkl')
        ...
        police_norm.to_pickle(CRIME_SAVE_CTX+'police_norm.pkl')
        print(pd.read_pickle(CRIME_SAVE_CTX+'police_norm.pkl'))
    ...
    def folium_example(self):
        ...
        map.save(CRIME_SAVE_CTX+"unemployment.html")
    ...
    def save_seoul_folium(self):
        ...
        map.save(CRIME_SAVE_CTX+"crime.html")


    def create_folium_data(self):
        police_pos = pd.read_pickle(CRIME_SAVE_CTX+'police_pos.pkl')
        police_norm = pd.read_pickle(CRIME_SAVE_CTX+'police_norm.pkl')
...
def set_json_from_df():
    df = pd.read_json(CRIME_DATA_CTX+'us-states.json')
    df.drop(df.index[[8,51]], inplace=True)
    df.to_json(CRIME_SAVE_CTX+'us-states.json', orient='index')
