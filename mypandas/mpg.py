...
MENU = ["종료", "mpg 앞부분 확인", "mpg 뒷부분 확인", "행, 열 출력", "데이터 속성 확인", "요약 통계량 출력",
                "문자 변수 요약 통계량 함께 출력", "manufacturer를 company로 변경",
                "test 변수 생성", # cty 와 hwy 변수를 머지(merge)하여 total 
...
class Mpg:
    ...
    def menu_11(self):
        displ_s = self.mpg.query('displ <= 4')
        displ_b = self.mpg.query('displ >= 5')
        avg_s = displ_s['hwy'].mean()
        avg_b = displ_b['hwy'].mean()
        if avg_s > avg_b:
    ...
    def menu_12(self):
        audi = self.mpg.query('manufacturer == "audi"')
        toyota = self.mpg.query('manufacturer == "toyota"')
        avg_a = audi['cty'].mean()
        avg_t = toyota['cty'].mean()
        if avg_a > avg_t:
